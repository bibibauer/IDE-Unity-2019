...
public class PointRenderer : MonoBehaviour
{
	...
	public bool highlightColor = true;
	...
	public int metadataVariable1 = 7;
	public int metadataVariable2 = 8;
	public int metadataVariable3 = 9;
	public int metadataVariable4 = 10;
	public int metadataVariable5 = 11;
	public int metadataVariable6 = 12;
	//public int metadataVariable7 = 12;

	// Target Minimum and maximum values for the data
	[Space(20)]
	public float xMin = 0;
	public float xMax = 125;
	public float yMin = 0;
	public float yMax = 60;
	public float zMin = 0;
	public float zMax = 125;
	...
	// Original MIN and MAX values from the data set
	private float xMinStart;
	private float xMaxStart;
	private float yMinStart;
	private float yMaxStart;
	private float zMinStart;
	private float zMaxStart;
	...
	void Start()
	{
		...
		xMaxStart = FindMaxValue(xDimensionName);
		yMaxStart = FindMaxValue(yDimensionName);
		zMaxStart = FindMaxValue(zDimensionName);

		// Get minimums of each axis, using FindMinValue method defined below
		xMinStart = FindMinValue(xDimensionName);
		yMinStart = FindMinValue(yDimensionName);
		zMinStart = FindMinValue(zDimensionName);
	...
	private void PlacePrefabPoints()
	{
		...
		for (var i = 0; i < rowCount; i++)
		{
			...
			float x = ExtensionMethods.Remap((Convert.ToSingle(pointList[i][xDimensionName])), xMinStart, xMaxStart, xMin, xMax) / xMax;
			float y = ExtensionMethods.Remap((Convert.ToSingle(pointList[i][yDimensionName])), yMinStart, yMaxStart, yMin, yMax) / xMax;
			float z = ExtensionMethods.Remap((Convert.ToSingle(pointList[i][zDimensionName])), zMinStart, zMaxStart, zMin, zMax) / xMax;
			...
			// depending on whether the Metdata Labels is toggled ON or OFF
			if (metadataLabels == true)
			{
				dataPoint.gameObject.transform.GetChild(0).GetComponent<TextMesh>().text =
					meta1Label + meta1 + "\n" + meta2Label + meta2 + "\n" + meta3Label + meta3 + "\n" + meta4Label + meta4 + "\n" + meta5Label + meta5 + "\n" + meta6Label + meta6;
				dataPoint.gameObject.transform.GetChild(1).GetComponent<TextMesh>().text =
					meta1Label + meta1 + "\n" + meta2Label + meta2 + "\n" + meta3Label + meta3 + "\n" + meta4Label + meta4 + "\n" + meta5Label + meta5 + "\n" + meta6Label + meta6;
			}
			...
			if (interview == 1)
			{
				dataPoint.gameObject.transform.GetChild(0).gameObject.transform.localPosition = new Vector3(x + 0.5f, y, z - 0.5f);
				dataPoint.gameObject.transform.GetChild(1).gameObject.transform.localPosition = new Vector3(x - 2.0f, y, z + 0.5f);
			}
		}
